...
/**
 * Mask email for display (keep first char and domain)
 */
...
async function collectAccountData(setName) {
  ...
  let recoveryEmail = '';
  while (true) {
    recoveryEmail = await prompt('Recovery email (opcional): ');
    if (recoveryEmail === '0') {
      info('\nOperação cancelada. Voltando ao menu principal...');
      rl.close();
      process.exit(0);
    ...
    if (!isValidEmail(recoveryEmail)) {
      error('Recovery email inválido! Digite um email válido ou deixe em branco para pular.');
      continue;
    }
    break;
  }

  // TOTP secret (Base32) opcional
  let totp = '';
  while (true) {
    const totpInput = await prompt('TOTP secret (Base32) (opcional): ');
    if (totpInput === '0') {
      info('\nOperação cancelada. Voltando ao menu principal...');
      rl.close();
      process.exit(0);
    ...
    // Normalizar (remover espaços e uppercase)
    const normalized = totpInput.replace(/\s+/g, '').toUpperCase();
    // Validar Base32 (A-Z2-7 and optional padding =)
    if (!/^[A-Z2-7]+=?=?$/.test(normalized)) {
      error('TOTP inválido. Deve ser um segredo Base32 (A-Z2-7). Tente novamente ou deixe em branco para pular.');
      continue;
    }
    totp = normalized;
    break;
...
function displayAccountSummary(account) {
  ...
  if (account.recoveryEmail) {
    log(`Recovery: ${maskEmail(account.recoveryEmail)}`);
  } else {
    log('Recovery: Não configurado');
  }

  // TOTP
  if (account.totp) {
    log('TOTP:     Configurado');
  } else {
    log('TOTP:     Não configurado');
  }
